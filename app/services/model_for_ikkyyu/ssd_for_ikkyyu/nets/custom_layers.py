...
def l1(bbox_pred, bbox_targets):
    with tf.name_scope('l1', [bbox_pred, bbox_targets]):
        return tf.minimum(tf.abs(tf.subtract(bbox_pred, bbox_targets)), 1)
#         return abs_smooth(tf.subtract(bbox_pred, bbox_targets))

    
def abs_smooth(x):
    """Smoothed absolute function. Useful to compute  n L1 smooth error.
