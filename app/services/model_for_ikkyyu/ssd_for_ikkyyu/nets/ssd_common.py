...
def tf_ssd_bboxes_encode_layer(labels,
    ...
    feat_scores = -10000 * tf.ones(shape, dtype=dtype)
    ...
    def jaccard_with_anchors(bbox):
        ...
        union_vol = vol_anchors - h * w \
            + (bbox[2] - bbox[0]) * (bbox[3] - bbox[1])

        h = int_ymax - int_ymin
        w = int_xmax - int_xmin
        jaccard = tf.div(tf.abs(h * w), union_vol)

        mask    = tf.logical_or(tf.less(h, 0), tf.less(w, 0))
        jaccard = tf.where(mask, -1 * jaccard, jaccard)
        return jaccard
    ...
    def body(i, feat_labels, feat_scores, feat_ymin,
             feat_xmin, feat_ymax, feat_xmax):
    ...
    # Encode features.
    feat_cy= (feat_cy - yref) / (href + 1e-8) / prior_scaling[0]
    feat_cx= (feat_cx - xref) / (wref + 1e-8) / prior_scaling[1]
    feat_h = tf.log((feat_h + 1e-8) / (href + 1e-8)) / prior_scaling[2]
    feat_w = tf.log((feat_w + 1e-8) / (wref + 1e-8)) / prior_scaling[3]
...
def tf_ssd_bboxes_encode(labels, bboxes, anchors,
                         num_classes, prior_scaling=[0.1, 0.1, 0.2, 0.2],
            ...
            with tf.name_scope('bboxes_encode_block_%i' % i):
                t_labels, t_loc, t_scores = \
                    tf_ssd_bboxes_encode_layer(labels, bboxes, anchors_layer,
                                               num_classes, prior_scaling, dtype)
...
def tf_ssd_bboxes_select_all_classes(predictions_net, localizations_net,
                                     select_threshold=None, scope=None):
    """Extract classes, scores and bounding boxes from network output layers.
    Batch-compatible: inputs are supposed to have batch-type shapes.
