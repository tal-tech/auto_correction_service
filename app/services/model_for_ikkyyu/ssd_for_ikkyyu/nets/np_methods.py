...
def bboxes_sort(classes, scores, bboxes, top_k=400):
    ...
    #     area = bboxes_area(bboxes[i])
    #     if area > 0.3 or area < 0:
    #         keep_bboxes[i] = False
    #     elif classes[i] ==1 and bboxes_area(bboxes[i]) > 0.15:
    #         keep_bboxes[i] = False
...
#             keep_overlap = np.logical_or(overlap < 0.8, classes[(i+1):] != classes[i])
    ...
    for i in range(scores.size):
        if classes[i] == 1:
            for j in range(scores.size):
                if classes[j] != 1 and is_contain(bboxes[i], bboxes[j]):
                    keep_bboxes[i] = False
    return classes[keep_bboxes], scores[keep_bboxes], bboxes[keep_bboxes]
