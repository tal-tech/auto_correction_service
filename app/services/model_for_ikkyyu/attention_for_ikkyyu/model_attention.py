...
device_conut = torch.cuda.device_count()
use_parallel = False

def get_model(model):
    if device_conut > 1 and use_parallel:
...
class BottleneckBlock(nn.Module):
    ...
    def __init__(self, input_size, growth_rate, dropout_rate=0.2):
        ...
        # self.norm1 = get_model(self.norm1)
        # self.relu = get_model(self.relu)
        # self.conv1 = get_model(self.conv1)
        # self.norm2 = get_model(self.norm2)
        # self.conv2 = get_model(self.conv2)
        # self.dropout = get_model(self.dropout)
...
class TransitionBlock(nn.Module):
    ...
    def __init__(self, input_size, output_size):
        ...
        # self.norm = get_model(self.norm)
        # self.relu = get_model(self.relu)
        # self.conv = get_model(self.conv)
        # self.pool = get_model(self.pool)
...
class Encoder(nn.Module):
    ...
    def __init__(
        ...
        self.norm0 = get_model(self.norm0)
        self.relu = get_model(self.relu)
        self.max_pool = get_model(self.max_pool)
        self.block1 = get_model(self.block1)
        self.trans1 = get_model(self.trans1)
        self.block2 = get_model(self.block2)
        self.trans2_norm = get_model(self.trans2_norm)
        self.trans2_relu = get_model(self.trans2_relu)
        self.trans2_conv = get_model(self.trans2_conv)
        self.trans2_pool = get_model(self.trans2_pool)
        self.multi_block = get_model(self.multi_block)
        self.block3 = get_model(self.block3)
