...
gpus = list(range(cfg["TRAIN"]["GPU_NUM"]))
...
def main():
    ...
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        if cfg["TRAIN"]["RESTORE"]:
            ...
            train_logger.info('ddd', ckpt.model_checkpoint_path)
            saver.restore(sess, ckpt.model_checkpoint_path)
            train_logger.info("done")
        elif os.path.exists(cfg["TRAIN"]["PRETRAINED_MODEL_DIR"]):
            sess.run(init)
            if cfg["TRAIN"]["PRETRAINED_MODEL_DIR"] is not None:
                train_logger.info('load pretrain model:', cfg["TRAIN"]["PRETRAINED_MODEL_DIR"])
        ...
        for step in range(cfg["TRAIN"]["MAX_STEPS"]):
            ...
            if step % cfg["TRAIN"]["SAVE_CHECKPOINT_STEPS"] == 0:
                saver.save(sess, cfg["TRAIN"]["CHECKPOINTS_OUTPUT_DIR"] + 'model.ckpt', global_step=global_step)
