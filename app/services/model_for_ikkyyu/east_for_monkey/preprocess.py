# coding=utf-8
import os
import json
from tqdm import tqdm
from lib.utils.utils import cfg
from PIL import Image
from PIL import ImageDraw
import numpy as np


# 判断文件是否为有效（完整）的图片
# 输入参数为文件路径
# 会出现漏检的情况1
def is_valid_image(pathfile):
    bValid = True
    try:
        Image.open(pathfile).verify()
    except:
        bValid = False
    return bValid


def get_rect(bbox):
    x1 = float(bbox[0][1:])
    y1 = float(bbox[1])
    x2 = float(bbox[2][1:])
    y2 = float(bbox[3])
    x3 = float(bbox[4][1:])
    y3 = float(bbox[5])
    x4 = float(bbox[6][1:])
    y4 = float(bbox[7])
    xy = np.array([[x1, y1], [x2, y2], [x3, y3], [x4, y4]])
    return xy


def preprocess():
    # 数据源路径
    data_dir = cfg["PREPROCESS"]["DATA_DIR"]
    # 源图片与对应json文件路径
    origin_image_dir = os.path.join(data_dir, cfg["PREPROCESS"]["ORIGIN_IMAGE_DIR"])
    origin_jsons_dir = os.path.join(data_dir, cfg["PREPROCESS"]["ORIGIN_JSONS_DIR"])

    # 训练图片及对应label所在路径,不存在则创建
    train_image_dir = os.path.join(data_dir, cfg["PREPROCESS"]["TRAIN_IMAGE_DIR"])
    train_label_dir = os.path.join(data_dir, cfg["PREPROCESS"]["TRAIN_LABEL_DIR"])
    if not os.path.exists(train_image_dir):
        os.mkdir(train_image_dir)
    if not os.path.exists(train_label_dir):
        os.mkdir(train_label_dir)

    # 是否生成原始训练和画框图片,默认TRUE
    is_gen_origin_img = cfg["PREPROCESS"]["IS_GEN_ORIGIN_IMG"]
    is_draw_gt_quad = cfg["PREPROCESS"]["IS_DRAW_GT_QUAD"]

    # 标注图与激活图文件路径,没有则创建
    show_gt_image_dir = os.path.join(data_dir, cfg["PREPROCESS"]["SHOW_GT_IMAGE_DIR"])
    show_act_image_dir = os.path.join(data_dir, cfg["PREPROCESS"]["SHOW_ACT_IMAGE_DIR"])
    if not os.path.exists(show_gt_image_dir):
        os.mkdir(show_gt_image_dir)
    if not os.path.exists(show_act_image_dir):
        os.mkdir(show_act_image_dir)

    # 读取json所有文件
    json_names = os.listdir(origin_jsons_dir)
    print("\n")
    print('Bobby:found %d origin jsons.' % len(json_names))

    # 遍历所有的json文件,将有用信息合并到images_datas
    images_datas = []
    for json_name in json_names:
        json_path = os.path.join(origin_jsons_dir, json_name)
        json_file = json.loads(open(json_path, encoding='utf-8').read(), encoding='bytes')
        # 将json文件中 true_message 的 image_datas 字段读取出来
        images_datas_temp = json_file['true_message']['image_datas']
        images_datas = images_datas + images_datas_temp

    # 遍历jsonList进行数据预处理
    for image_datas in tqdm(images_datas):

        # 取出图片名字
        o_img_fname = image_datas['pic_name'].split('/')[2]

        # 检测图片在本地是否存在
        if not os.path.exists(os.path.join(origin_image_dir, o_img_fname)):
            print("\n")
            print("Bobby: not exist image is " + os.path.join(origin_image_dir, o_img_fname))
            continue

        # 检测文件是否能正常打开
        if is_valid_image(os.path.join(origin_image_dir, o_img_fname)) is False:
            print("\n")
            print("Bobby: error image is " + os.path.join(origin_image_dir, o_img_fname))
            continue

        # 打开图片,并对图片进行处理
        with Image.open(os.path.join(origin_image_dir, o_img_fname)) as im:
            # 获取训练图片设定 宽 与 高
            d_wight = cfg["TRAIN"]["MAX_IMAGE_LARGE_SIDE"]
            d_height = cfg["TRAIN"]["MAX_IMAGE_LARGE_SIDE"]

            im = im.resize((d_wight, d_height), Image.NEAREST).convert('RGB')
            show_gt_im = im.copy()

            # 画图
            draw = ImageDraw.Draw(show_gt_im)
            anno_list = image_datas['mark_datas']

            # 创建文件生成图片对应的TXT,并保存边框数据
            txt_file = os.path.join(train_label_dir, os.path.splitext(o_img_fname)[0] + '.txt')
            # print("\n bobby:txt_file:" + txt_file)
            with open(txt_file, 'w') as f:
                for anno, i in zip(anno_list, range(len(anno_list))):
                    bbox = anno['marked_path']
                    bbox = bbox.split()
                    xy_list = get_rect(bbox)

                    xy_list[:, 0] = xy_list[:, 0] * d_wight
                    xy_list[:, 1] = xy_list[:, 1] * d_height

                    # 在图片上GT画框
                    if is_draw_gt_quad:
                        draw.line([tuple(xy_list[0]), tuple(xy_list[1]),
                                   tuple(xy_list[2]), tuple(xy_list[3]),
                                   tuple(xy_list[0])
                                   ],
                                  width=2, fill='green')

                    # 样例: 374,1,494,0,492,85,372,86,###
                    line_label = "{},{},{},{},{},{},{},{},###".format(str(int(xy_list[0][0])), str(int(xy_list[0][1])),
                                                                      str(int(xy_list[1][0])), str(int(xy_list[1][1])),
                                                                      str(int(xy_list[2][0])), str(int(xy_list[2][1])),
                                                                      str(int(xy_list[3][0])), str(int(xy_list[3][1])))

                    f.write(line_label + '\n')

            # 生成训练图片,存在训练数据文件夹
            if is_gen_origin_img:
                im.save(os.path.join(train_image_dir, o_img_fname))

            # 生成GT画框的图像,并保存
            if is_draw_gt_quad:
                show_gt_im.save(os.path.join(show_gt_image_dir, o_img_fname))


if __name__ == '__main__':
    preprocess()
