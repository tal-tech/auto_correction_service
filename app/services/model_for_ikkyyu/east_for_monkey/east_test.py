...
class EastTest():
    ...
    def inference(self, model_path=None):
        ...
        if model_path is None:
            ...
            print('Restoring from {}...'.format(model_path), end=' ')
    ...
    def test(self, img_path):
        ...
        resized_img, (ratio_h, ratio_w) = self.__resize_img(img, 720)
...
if __name__ == "__main__":
    ...
    #     print('dwadawdwaddddddddddddd     ',i)
    #     east.test(os.path.join(img_dir, i))


    ckpt_path = '/share/zzh/east_data/ckpt/v0.7.9'
    ...
    for model_path in ckpt.all_model_checkpoint_paths:
        ...
        model_val_res_dir = os.path.join(ckpt_path, 'val', model_name)
        if not os.path.exists(model_val_res_dir):
            os.makedirs(model_val_res_dir)

        east = EastTest()
        east.inference(model_path)
        # 测试图片文件夹路径d
        img_dir = cfg["TEST"]["TEST_DIR"]
        # 读取文件夹下路径
        img_list = os.listdir(img_dir)
        ...
        # 结果图文件夹
        if os.path.exists(os.path.join(model_val_res_dir, 'result_img')):
            shutil.rmtree(os.path.join(model_val_res_dir, 'result_img'))
        os.makedirs(os.path.join(model_val_res_dir, 'result_img'))

        if os.path.exists(os.path.join(model_val_res_dir, 'result_txt')):
            shutil.rmtree(os.path.join(model_val_res_dir, 'result_txt'))
        os.makedirs(os.path.join(model_val_res_dir, 'result_txt'))

        # 得分图路径
        if os.path.exists(os.path.join(model_val_res_dir, 'score_map')):
            shutil.rmtree(os.path.join(model_val_res_dir, 'score_map'))
        os.makedirs(os.path.join(model_val_res_dir, 'score_map'))
