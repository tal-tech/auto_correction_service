...
from lib.dataset.data_util import GeneratorEnqueuer
...
def get_images():
    ...
    for ext in ['jpg', 'png', 'jpeg', 'JPG']:
        files.extend(glob.glob(
            os.path.join(cfg["TRAIN"]["TRAINING_DATA_DIR"], '*.{}'.format(ext))))
    return files


def load_annoataion(label_file):
    '''
    ...
    assert len(bbox_list) != 0, "{} is error label".format(label_file)
...
def generator(input_size=512, batch_size=32,
    ...
    print('{} training images in {}'.format(image_list.shape[0], cfg["TRAIN"]["TRAINING_DATA_DIR"]))
    ...
    while True:
        ...
        for i in index:
            try:
                ...
                # txt_fn = os.path.join(cfg["TRAIN"]["TRAINING_DATA_LABEL_DIR"], 'gt_' + img_name + '.txt')
                txt_fn = os.path.join(cfg["TRAIN"]["TRAINING_DATA_LABEL_DIR"], img_name + '.txt')
                ...
                if np.random.rand() < background_ratio:
                    ...
                    # print('222222')
                if vis:
                    # print('emmmm',img_name)
                    # print(os.path.join('./test/', img_name+'.jpg'))
                    cv2.imwrite(os.path.join('./test/', img_name + '.jpg'), im)
                    vs_map = score_map.copy()
                    vs_map[np.where(vs_map == 1)] = 255
                    cv2.imwrite(os.path.join('./test/score/', img_name + '_score.jpg'), vs_map * training_mask[::, ::])
                    cv2.imwrite(os.path.join('./test/ge/', img_name + '_g0.jpg'), geo_map[::, ::, 0])
                    cv2.imwrite(os.path.join('./test/ge/', img_name + '_g1.jpg'), geo_map[::, ::, 1])
                    cv2.imwrite(os.path.join('./test/ge/', img_name + '_g2.jpg'), geo_map[::, ::, 2])
                    cv2.imwrite(os.path.join('./test/ge/', img_name + '_rbg.jpg'), geo_map[::, ::, 0:3])
