...
namespace lanms_adaptor {
	...
	std::vector<std::vector<double>> polys2floats(const std::vector<lanms::Polygon> &polys) {
		std::vector<std::vector<double>> ret;
		for (size_t i = 0; i < polys.size(); i ++) {
			auto &p = polys[i];
			auto &poly = p.poly;
			ret.emplace_back(std::vector<double>{
	 ...
	 */
	std::vector<std::vector<double>> merge_quadrangle_n9(
			py::array_t<double, py::array::c_style | py::array::forcecast> quad_n9,
			float iou_threshold) {
		auto pbuf = quad_n9.request();
		if (pbuf.ndim != 2 || pbuf.shape[1] != 9)
			throw std::runtime_error("quadrangles must have a shape of (n, 9)");
		auto n = pbuf.shape[0];
		double* ptr = static_cast<double *>(pbuf.ptr);
